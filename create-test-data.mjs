  ...
  try {
    ...
    if (existingAccount) {
      ...
      await prisma.$transaction(async (tx) => {
        await tx.transaction.deleteMany({ where: { accountId: existingAccount.id } });
        await tx.valuationSnapshot.deleteMany({ where: { accountId: existingAccount.id } });
        await tx.account.delete({ where: { id: existingAccount.id } });
      });
    }
