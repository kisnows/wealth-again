...
4. 评估并下线 `/reports` 中收入相关入口，将流量统一汇聚到 `/income`。
...
- 移除 `/reports` 收入相关页面，确保导航仅指向 `/income`。
...
- Playwright/Chrome DevTools 打开 http://localhost:4000 验证 `/income`、`/income/recalc-status` 导航。
...
- [x] `/reports` 收入/资产页面改版合并至核心路由，保留 `/income` 与 `/accounts` 作为唯一入口。
