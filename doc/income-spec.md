...
根据“页面功能不冗余”原则，原 `/reports/income` 页面被移除，其功能被整合进 `/income` 页面。现在，`/income` 页面是管理和分析收入的**唯一中心**，顶部提供紧凑的“核心配置入口”快速链接（工资、奖金、长期现金、税务等），其下依次包含以下视图：
...
3.  **月度收入记录 (Monthly Records)**
    ...
    - 前端时间线在每条实际记录右侧提供“人工调整”入口，可录入人工税后净收入与备注，也支持恢复为系统计算结果。

前端通过单一的 `IncomeAnalyticsPanel` 时间线组件呈现上述聚合、时序与月度记录视图，并在同一表格内标记预测月份。
