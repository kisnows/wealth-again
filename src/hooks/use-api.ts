...
import { useCache, useLoading, useNotifications } from "@/lib/app-state";
...
export function useTransactions(accountId?: string) {
  const url = accountId
    ? `/api/investment/transactions?accountId=${accountId}`
...
export function useApiMutation<TData = any, TVariables = any>(
  ...
  const mutate = useCallback(
    async (variables: TVariables, method: "POST" | "PUT" | "DELETE" = "POST") => {
      if (!session) {
