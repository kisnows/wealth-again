...
export async function ensureIdempotent(req: Request, userId?: string, hash?: string) {
  ...
  await prisma.idempotencyKey.create({ data: { key, userId, hash: hash || null } });
  return { key, existed: false } as const;
}

export async function markIdempotencyUsed(key: string | null) {
  if (!key) return;
  await prisma.idempotencyKey.update({ where: { key }, data: { usedAt: new Date() } });
}
