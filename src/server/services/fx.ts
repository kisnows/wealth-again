...
export async function getFxRate(params: { base: string; quote: string; asOf?: Date }) {
  const base = params.base;
  const quote = params.quote;
  const on = params.asOf;
  if (!on) {
    return prisma.fxRate.findFirst({ where: { base, quote }, orderBy: { asOf: "desc" } });
  }
  const first = await prisma.fxRate.findFirst({ where: { base, quote, asOf: { lte: on } }, orderBy: { asOf: "desc" } });
  if (first) return first;
  return prisma.fxRate.findFirst({ where: { base, quote, asOf: on } });
}

export async function convert(amount: number, from: string, to: string, asOf: Date) {
