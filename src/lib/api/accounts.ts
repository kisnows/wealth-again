...
export function useAccountTimeseries(id: string | undefined, metric: "valuation" | "principal" = "valuation", from?: string, to?: string) {
  if (!id) return { data: undefined, error: undefined, isLoading: false } as const;
  const params = new URLSearchParams({ metric });
  if (from) params.set("from", from);
  if (to) params.set("to", to ?? "");
  const key = `/api/v1/accounts/${id}/timeseries?${params.toString()}`;
  return useSWR<{ points: Array<{ asOf: string; value: number }> }>(key, getJson);
}
...
export async function updateAccount(id: string, patch: Partial<Pick<Account, "name" | "subType" | "description" | "status">>) {
...
export async function postWithdraw(input: { accountId: string; amount: number; occurredAt: string; note?: string }) {
...
export async function postValuation(input: { accountId: string; asOf: string; totalValue: number; currency?: string; fxRateId?: string; note?: string }) {
