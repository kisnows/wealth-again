...
export function TransferDialog({ defaultFromId, defaultToId }: { defaultFromId?: string; defaultToId?: string }) {
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({ fromAccount: defaultFromId ?? "", toAccount: defaultToId ?? "", amount: "", occurredAt: "", note: "" });
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    toast.success("转账成功");
    setOpen(false);
    setForm({ fromAccount: "", toAccount: "", amount: "", occurredAt: "", note: "" });
  };
  return (
      ...
      <DialogContent>
        ...
        <form onSubmit={onSubmit} className="grid gap-3">
          <div className="grid gap-1">
            <Label>转出账户</Label>
            <Select value={form.fromAccount} onValueChange={(v) => setForm((s) => ({ ...s, fromAccount: v }))} disabled={isLoading}>
          ...
          <div className="grid gap-1">
            <Label>转入账户</Label>
            <Select value={form.toAccount} onValueChange={(v) => setForm((s) => ({ ...s, toAccount: v }))} disabled={isLoading}>
              <SelectTrigger><SelectValue placeholder="选择账户" /></SelectTrigger>
              <SelectContent>
                {(accounts ?? []).map((a) => (
                  <SelectItem key={a.id} value={a.id}>{a.name} ({a.baseCurrency})</SelectItem>
          ...
          <div className="grid gap-1">
            ...
            <Input name="occurredAt" type="datetime-local" value={form.occurredAt} onChange={onChange} />
          </div>
          <div className="grid gap-1">
            <Label>Note</Label>
