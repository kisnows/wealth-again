...
export async function GET(req: NextRequest) {
  ...
  if (!country || !taxYear) return NextResponse.json({ error: "country & taxYear required" }, { status: 400 });
  const items = await prisma.taxBracket.findMany({ where: { country, taxYear }, orderBy: { position: "asc" } });
...
export async function PUT(req: Request) {
  ...
  const { key, existed } = await ensureIdempotent(req, undefined, `taxbr:${items.length}`);
  if (existed) return NextResponse.json({ error: "Idempotency key reused" }, { status: 409 });
  for (const it of items) {
    await prisma.taxBracket.upsert({
      where: { country_taxYear_position: { country: it.country, taxYear: it.taxYear, position: it.position } },
      update: { threshold: it.threshold, taxRate: it.taxRate, quickDeduction: it.quickDeduction },
      create: { country: it.country, taxYear: it.taxYear, position: it.position, threshold: it.threshold, taxRate: it.taxRate, quickDeduction: it.quickDeduction },
    });
  }
