 ...
 * - 批量 upsert 社保规则。
 * - 入参: Array<{ city: string|id, startDate: ISO, endDate?: ISO, baseMin, baseMax, ratePension, rateMedical, rateUnemployment }>
 */
...
export async function GET(req: NextRequest) {
  ...
  const rule = await prisma.cityRuleSS.findFirst({ where: { cityId: city.id, startDate: { lte: onDate }, OR: [{ endDate: null }, { endDate: { gt: onDate } }] }, orderBy: { startDate: "desc" } });
...
export async function PUT(req: Request) {
  ...
  const { key, existed } = await ensureIdempotent(req, undefined, `ss:${items.length}`);
  ...
  for (const it of items) {
    ...
    // overlap check (JS 级别)
    const existing = await prisma.cityRuleSS.findMany({ where: { cityId: (city as any).id } });
