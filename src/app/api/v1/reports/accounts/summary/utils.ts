...
export async function computeAccountSummary(displayCurrency?: string, userId?: string) {
  ...
  return accounts.map((a) => {
    const principal = a.txnLines.reduce((s, l) => s + Number(l.amount), Number(a.initialBalance));
    const valuation = a.accountType === "SAVINGS" ? principal : Number(a.valuations[0]?.totalValue ?? principal);
