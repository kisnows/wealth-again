...
export async function POST(req: NextRequest) {
  try {
    ...
    if (lines.length < 2) return NextResponse.json({ error: "no data" }, { status: 400 });
    ...
    for (const [k, v] of Object.entries(map)) {
      const idx = headers.findIndex((h: string) => h.toLowerCase() === String(v).toLowerCase());
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      try {
        ...
        const amount =
          headerIndex["amount"] != null ? Number(cols[headerIndex["amount"]]) : undefined;
        const currency = headerIndex["currency"] != null ? cols[headerIndex["currency"]] : "CNY";
