import { type NextRequest, NextResponse } from "next/server";
import { ZodError } from "zod";
import { prisma } from "@/lib/prisma";
import { fetchHangzhouParams } from "@/lib/sources/hz-params";
import { createTaxService, taxParamsSchema } from "@/lib/tax";
import { taxConfigSchema } from "@/lib/tax/types";

const taxService = createTaxService(prisma);

export async function POST(req: NextRequest) {
  try {
    let body: any;
    try {
      body = await req.json();
    } catch (e) {
      return NextResponse.json({ error: "invalid json body" }, { status: 400 });
    }

    // 检查是否是新格式的税务配置
    if (body.taxBrackets && body.socialInsurance) {
      // 新格式：使用新的税务服务
      const parsed = taxConfigSchema.parse(body);
      await taxService.saveTaxConfig(parsed);

      return NextResponse.json(
        {
          success: true,
          message: "税务配置已保存",
          city: parsed.city,
          effectiveFrom: parsed.effectiveFrom,
        },
        { status: 201 },
      );
    } else {
      // 兼容旧格式：导入杭州参数
      const params = taxParamsSchema.parse(body);
      const result = await taxService.importHangzhouParams(params as any);

      // 同时保存到Config表（向后兼容）
      const key = `tax:${params.city}:${params.year}`;
      const effectiveFrom = body.effectiveFrom
        ? new Date(body.effectiveFrom)
        : new Date(`${params.year}-01-01`);

      const rec = await prisma.config.create({
        data: {
          key,
          value: JSON.stringify(params),
          effectiveFrom,
        },
      });

      return NextResponse.json(
        {
          id: rec.id,
          key,
          success: result.success,
          message: "参数已导入并保存到新的税务配置表",
        },
        { status: 201 },
      );
    }
  } catch (err: unknown) {
    if (err instanceof ZodError) {
      return NextResponse.json({ error: "invalid body", issues: err.issues }, { status: 400 });
    }
    console.error("POST /api/config/tax-params error:", err);
    return NextResponse.json({ error: "internal error" }, { status: 500 });
  }
}

export async function GET(req: NextRequest) {
  const { searchParams } = new URL(req.url);
  const city = searchParams.get("city");
  const year = searchParams.get("year");
  const page = Number(searchParams.get("page") || "1");
  const pageSize = Number(searchParams.get("pageSize") || "50");

  if (!city || !year) {
    return NextResponse.json({ error: "city & year required" }, { status: 400 });
  }

  try {
    // 优先从新的税务配置表获取数据
    const date = new Date(`${year}-07-01`); // 使用年中时间点查询
    const currentConfig = await taxService.getCurrentTaxConfig(city, date);

    if (currentConfig) {
      return NextResponse.json({
        city,
        year: Number(year),
        config: currentConfig,
        source: "new_tax_system",
        message: "数据来自新的税务配置系统",
      });
    }

    // 如果新系统没有数据，回退到旧的Config表
    const key = `tax:${city}:${year}`;
    const skip = (page - 1) * pageSize;

    const [total, latest, records] = await Promise.all([
      prisma.config.count({ where: { key } }),
      prisma.config.findFirst({
        where: { key },
        orderBy: { effectiveFrom: "desc" },
      }),
      prisma.config.findMany({
        where: { key },
        orderBy: { effectiveFrom: "desc" },
        skip,
        take: pageSize,
      }),
    ]);

    if (!latest) {
      // Auto bootstrap for Hangzhou if not found
      if (city === "Hangzhou") {
        const params = await fetchHangzhouParams({ year: Number(year), city });

        // 保存到新系统
        await taxService.importHangzhouParams(params as any);

        // 同时保存到旧系统（兼容性）
        const rec = await prisma.config.create({
          data: {
            key,
            value: JSON.stringify(params),
            effectiveFrom: new Date(`${year}-01-01`),
          },
        });

        return NextResponse.json({
          key,
          params: params,
          total: 1,
          page: 1,
          pageSize: pageSize,
          records: [rec],
          source: "auto_bootstrap",
          message: "自动导入杭州税务参数",
        });
      } else {
        return NextResponse.json({ error: "not found" }, { status: 404 });
      }
    }

    return NextResponse.json({
      key,
      params: JSON.parse(latest.value),
      total,
      page,
      pageSize,
      records,
      source: "legacy_config",
      message: "数据来自原有配置系统",
    });
  } catch (error) {
    console.error("GET /api/config/tax-params error:", error);
    return NextResponse.json({ error: "internal error" }, { status: 500 });
  }
}
