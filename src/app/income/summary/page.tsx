...
export default function IncomeSummaryPage() {
  ...
  useEffect(()=>{ fetch(`/api/income/summary?year=${year}`).then(r=>r.json()).then(setData); }, [year]);

  const records = data.records || [];
  const cumulative = records.reduce((acc:any, r:any)=>{
  ...
  return (
    <div className="container mx-auto py-8 space-y-6">
      ...
      <IncomeSummary />
      <Card>
        <CardHeader><CardTitle>图表预览（柱状：税前/税后；折线：累计）</CardTitle></CardHeader>
        <CardContent>
          <div className="text-sm text-gray-500 mb-2">简版：可替换为 recharts 图</div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr className="border-b"><th className="text-left py-2">月份</th><th className="text-left py-2">税前</th><th className="text-left py-2">税后</th><th className="text-left py-2">累计税前</th><th className="text-left py-2">累计税后</th></tr></thead>
              <tbody>
                {records.reduce((rows:any[], r:any, idx:number)=>{
